...
export async function resolveUserToAccountId(
  ...
  if (userIdentifier.toLowerCase() === 'unassigned' || userIdentifier.toLowerCase() === 'none') {
    throw new McpError(
      ...
      'Use null or empty string to unassign, not "unassigned"'
    );
  }
  ...
  try {
    ...
    if (exactMatch) {
      console.error(
        `Found exact display name match: ${exactMatch.displayName} (${exactMatch.accountId})`
      );
    ...
    if (exactMatch) {
      console.error(
        `Found email match: ${exactMatch.emailAddress} -> ${exactMatch.displayName} (${exactMatch.accountId})`
      );
      return exactMatch.accountId;
    }
...
export async function getUserByAccountId(
  ...
  try {
    ...
    throw new McpError(ErrorCode.InvalidRequest, `User with account ID "${accountId}" not found`);
  }
}
