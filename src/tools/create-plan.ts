...
## 🎯 Issue Sources
${toolArgs.issueSources
  .map((source, index) => `${index + 1}. **${source.type}** (ID: ${source.value})`)
  .join('\n')}
...
${
  toolArgs.crossProjectReleases?.length
    ? `## 🔗 Cross-Project Releases
${toolArgs.crossProjectReleases
  .map(
    (release, index) => `${index + 1}. **${release.name}** (${release.releaseIds.length} releases)`
  )
  .join('\n')}`
    : ''
}
...
${
  toolArgs.customFields?.length
    ? `## 🔧 Custom Fields
${toolArgs.customFields
  .map(
    (field, index) =>
...
${
  toolArgs.permissions?.length
    ? `## 🔐 Permissions
${toolArgs.permissions
  .map(
    (perm, index) =>
