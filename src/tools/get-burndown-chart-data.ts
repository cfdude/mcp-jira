...
Sprint has not started yet. Burndown data will be available once the sprint begins.`,
    ...
    const endDate = sprint.endDate ? new Date(sprint.endDate) : new Date(startDate.getTime() + (14 * 24 * 60 * 60 * 1000)); // Default 2 weeks
    const now = new Date();
    ...
    if (daysElapsed > 0 && actualRemaining > 0) {
      ...
      const projectedDaysToComplete = currentBurnRate > 0 ? actualRemaining / currentBurnRate : Infinity;
