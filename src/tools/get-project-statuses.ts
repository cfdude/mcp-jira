...
export async function handleGetProjectStatuses(
  ...
  try {
    ...
    const workflowsResponse = await axiosInstance.get(
...
${Object.entries(categoryStats)
  ...
  .map(([category, count]) => {
    ...
    switch (category.toLowerCase()) {
      case "to do": emoji = "🔵"; break;
      case "in progress": emoji = "🟡"; break;
...
${Object.entries(statusesByIssueType).map(([issueTypeName, typeData]: [string, any]) => 
  `### ${issueTypeName} (${typeData.statuses.length} statuses)
${typeData.statuses.map((status: any) => {
  let emoji = "⚪";
  switch (status.categoryKey) {
    case "new": emoji = "🔵"; break;
    case "indeterminate": emoji = "🟡"; break;
    case "done": emoji = "🟢"; break;
  }
...
## 📈 All Unique Statuses
${Array.from(allStatuses).sort().map(statusName => {
