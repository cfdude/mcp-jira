...
export async function handleListInstances(args: ListInstancesArgs) {
  try {
    ...
    if (instances.length === 0) {
      ...
      result += "To configure multiple instances, create a .jira-config.json file with this structure:\n";
      result += "```json\n";
    ...
    result += "1. **Explicit Project Mapping**: If a project is configured in the `projects` section, that instance will be used\n";
    result += "2. **Instance Project Lists**: If an instance lists the project in its `projects` array, that instance will be used\n";
    result += "3. **Default Instance**: If configured, the default instance will be used for unmapped projects\n";
    result += "4. **Single Instance**: If only one instance is configured, it will be used for all projects\n\n";
